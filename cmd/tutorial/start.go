...
func newStartCommand(cli labcli.CLI) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		ValidArgsFunction: completion.TutorialNames(cli),
...
func runStartTutorial(ctx context.Context, cli labcli.CLI, opts *startOptions) error {
	...
	go func() {
		...
		} else if err := playConn.WaitPlayReady(startTutorialTimeout, spin); err != nil {
		...
		case ev := <-eventCh:
			...
			case EventTutorialReady:
				if !opts.noSSH {
					...
					sess, errCh, err = ssh.StartSSHSession(ctx, cli, tut.Play, opts.machine, opts.user, nil, opts.forwardAgent)
					if err != nil {
