...
func newStartCommand(cli labcli.CLI) *cobra.Command {
	...
	flags.StringVar(
		...
		"ide",
		"",
		fmt.Sprintf(`Open the challenge playground in the IDE by specifying the IDE name (supported: %s)`, ide.SupportedList()),
	)
	flags.BoolVar(
...
func runStartChallenge(ctx context.Context, cli labcli.CLI, opts *startOptions) error {
	...
	if opts.machine, err = chal.Play.ResolveMachine(opts.machine); err != nil {
		return err
	}
	if opts.user, err = chal.Play.ResolveUser(opts.machine, opts.user); err != nil {
		return err
	}

	if !opts.noOpen {
		browser.OpenWithFallbackMessage(cli, chal.PageURL)
	}
